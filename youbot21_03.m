    ...
    elseif strcmp(fsm, 'rotate'),
        ...
        if (abs(rotVel) < .1/180*pi) && (abs(angdiff(prevOri, youbotEuler(3))) < .01/180*pi),
            ...
            if any(map(sub2ind(size(map), path(:,2), path(:,1)))) 
                ...
                prev_dist_target = 7.5*sqrt(2); % dist max of the room
                cnt_drive = 0;
                fsm = 'drive';
                fprintf('Drive \n');
            end
...
%         end
        ...
        curr_dist_target = sqrt( (youbotPos(1)-q_ref(1,cnt))^2 + (youbotPos(1)-q_ref(1,cnt))^2 )
        errDr = curr_dist_target - prev_dist_target;
        if cnt_drive == 15;
            prevErrRot = errRot;
            cnt_drive = 0;
        end
        ...
        if (abs(curr_dist_target) < 0.1) %nextPDrive(2)
            forwBackVel = 0;
            h = youbot_drive(vrep, h, forwBackVel, leftRightVel, rotVel);
            if cnt < length(via)
                cnt = cnt +1;
