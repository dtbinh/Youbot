% Function that compute the path
% goal = [goal_x, goal_y]
% start = [start_x, start_y]
% map = the map
% (no need to flip)

%  Assumption from the statement, the room is surrounded by a wall 

% Maybe to improve : 
% Verify that goal is not on a wall

function [path] = ourPathPlanner(map, goal, start)
    ...
    map_cost(:,1) = Inf;
    map_cost(end,1) = Inf;
    G = sub2ind(size(map), goal(2), goal(1));
    map_cost(G) = 0;
    stack = [];
    stack = [stack G];
    ...
    S = sub2ind(size(map), start(2), start(1));
    curr_point = S;
    path = [S];
    while true,
        % UP
