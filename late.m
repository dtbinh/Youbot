...
while true,
    ...
    if strcmp(fsm, 'path'),
        ...
        via = path(ind_angles,:);
        via = [via; goal];
        %via = (path((path(1:end-1,1) ~= path(2:end,1)) & (path(1:end-1,2) ~= path(2:end,2)),:))
        %q = [path, zeros(length(path),1), ones(length(path),1)/cell].';
        %q_ref = (RTr\q)*cell;
        q = (via*cell).';
        %q = (path*cell).';
        ...
        cnt = 1;
        TransAbsToRobot = [1 0 youbotPos(1); 0 1 youbotPos(2); 0 0 1];
        gRot = homtrans(inv(TransAbsToRobot), q_ref(:,cnt));
        angl = -atan2(gRot(1),gRot(2));
        fsm = 'rotate';
        fprintf('Rotate \n');
    ...
    elseif strcmp(fsm, 'rotate'),
        ...
        anglSensor = -sign(youbotEuler(3))*(pi-abs(youbotEuler(3)));
        errRot = angdiff(angl, anglSensor);
